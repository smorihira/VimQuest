...
const TEXT_OBJ_TARGETS = new Set<string>([
  'w',
  '"',
  "'",
  '(',
...
export class CommandParser {
  ...
  private transition(key: string): ParseResult | null {
    // Backspace removes last char from pending buffer (or cancels if buffer is 1 char)
    if (key === 'Backspace' && this.state !== 'idle' && this.state !== 'searchInput') {
      const prevBuffer = this.buffer.slice(0, -1)
      this.reset()
      if (prevBuffer.length === 0) return null
      // Re-feed remaining characters to rebuild parser state
      for (const ch of prevBuffer) {
        const result = this.transition(ch)
  ...
  private handleIdle(key: string): ParseResult | null {
    ...
    // In visual mode, these are instant commands (operate on selection)
    if (
  ...
  private handleOperatorPending(key: string): ParseResult | null {
    ...
    if (key === this.operator) {
      ...
      return this.emit({ raw, operator: this.operator, count, valid: true }, 1)
    }
  ...
  private handleRPending(key: string): ParseResult | null {
    ...
    if (key.length === 1) {
      const raw = this.buffer
      const count = mergeCount(this.countPrefix, this.countAfterOp)
      return this.emit({ raw, char: key, count, valid: true }, 1)
    }
  ...
  private handleFPending(key: string): ParseResult | null {
    ...
    if (key.length === 1) {
      ...
      const motionLetter = this.operator
        ...
        : this.countPrefix !== undefined
          ? raw[String(this.countPrefix).length]
          : raw[0]
      if (this.operator) {
        return this.emit(
          {
      ...
      return this.emit({ raw, motion: motionLetter as Motion, char: key, count, valid: true }, 1)
    }
  ...
  private handleTextobjPending(key: string): ParseResult | null {
    ...
    if (TEXT_OBJ_TARGETS.has(key)) {
      ...
      const count = mergeCount(this.countPrefix, this.countAfterOp)
      return this.emit({ raw, operator: this.operator, textObject: textObj, count, valid: true }, 1)
    }
  ...
  private handleGuPending(key: string): ParseResult | null {
    ...
    if (SIMPLE_MOTIONS.has(key)) {
      ...
      const count = mergeCount(this.countPrefix, this.countAfterOp)
      return this.emit({ raw, operator: 'gu', motion: key as Motion, count, valid: true }, 1)
    }
 ...
 */
export function parseKeys(keys: string[], availableCommands?: string[]): ParseResult | null {
