...
function resolveMotion(
    ...
    switch (motion) {
        ...
        case 'B': return moveWORDBackward(state, count)
        case 'E': return moveWORDEnd(state, count)
        case 'f': return char ? findCharForward(state, char, count) : null
        case 'F': return char ? findCharBackward(state, char, count) : null
        case 't': return char ? tilCharForward(state, char, count) : null
        case 'T': return char ? tilCharBackward(state, char, count) : null
        case '0': return moveLineStart(state)
        case '$': return moveLineEnd(state)
        case '^': return moveFirstNonBlank(state)
        case 'G': return moveToLine(state, rawCount)
        case 'gg': return moveToFirstLine(state, rawCount)
        case '%': return matchBracket(state)
        case 'gg': return moveToFirstLine(state, rawCount)
        case 'gj': return moveDown(state, count)
        case 'gk': return moveUp(state, count)
        case ';': {
        ...
        case ',': {
            ...
            return resolveMotion(state, reverse[state.lastFindMotion.motion], count, state.lastFindMotion.char)
        }
        default: return null
    }
}
...
function executeMotion(state: EditorState, cmd: Command): EditorState {
    ...
    if ((motion === 'f' || motion === 'F' || motion === 't' || motion === 'T') && cmd.char) {
        newState = { ...newState, lastFindMotion: { motion: motion as 'f' | 'F' | 't' | 'T', char: cmd.char } }
    }
    return newState
}
...
function executeOperatorMotion(state: EditorState, cmd: Command): EditorState {
    ...
    // Store last f/F/t/T for ; and , repeat
    if ((motion === 'f' || motion === 'F' || motion === 't' || motion === 'T') && cmd.char) {
        state = { ...state, lastFindMotion: { motion: motion as 'f' | 'F' | 't' | 'T', char: cmd.char } }
    }
    ...
    if (operator === 'c') {
        ...
        return { ...state, text: newText, cursor: newCursor, mode: 'insert', registers: { ...state.registers, '': deleted } }
    }
...
function resolveTextObject(
    ...
    switch (textObj) {
        case 'iw': {
            ...
            const predicate = isW ? isWordChar : isSp ? isSpace : (c: string) => !isWordChar(c) && !isSpace(c)
            let from = curOffset
            let to = curOffset
...
function findInsideDelimiters(flat: string, curOffset: number, delim: string): { from: number; to: number } | null {
    ...
    if (delim === '"' || delim === "'") {
        ...
        if (start === curOffset) {
            ...
            for (let i = start + 1; i < flat.length; i++) {
                if (flat[i] === delim) { end = i; break }
            }
            if (end === -1) {
                // This was closing quote, look backward for opening
                for (let i = start - 1; i >= 0; i--) {
                    if (flat[i] === delim) { start = i; end = curOffset; break }
                }
            }
        } else {
...
function executeSubstituteLine(state: EditorState): EditorState {
    ...
    const newText = join(newLines)
    return { ...state, text: newText, cursor: { line: state.cursor.line, col: indent.length }, mode: 'insert', registers: { ...state.registers, '': line } }
}
...
function executeOperatorTextObject(state: EditorState, cmd: Command): EditorState {
    ...
    if (operator === 'c') {
        ...
        const result = { ...state, text: newText, cursor: clampCursor(newText, cCursor, 'insert'), mode: 'insert' as VimMode, registers: { ...state.registers, '': deleted } }
        return result
    }
...
export function executeCommand(state: EditorState, cmd: Command): EditorState {
    ...
    if (state.mode === 'visual') {
        ...
        if (cmd.motion && !cmd.operator) {
            if (cmd.motion === 'n') return { ...executeSearchNext(state), mode: 'visual', visualStart: state.visualStart, visualType: state.visualType }
            if (cmd.motion === 'N') return { ...executeSearchPrev(state), mode: 'visual', visualStart: state.visualStart, visualType: state.visualType }
            const newState = executeMotion(state, cmd)
            return { ...newState, mode: 'visual', visualStart: state.visualStart, visualType: state.visualType }
        }
        ...
        if (raw === 'd' || raw === 'x') {
            ...
            if (state.visualType === 'line') {
                ...
                return { ...result, mode: 'normal', visualStart: undefined, visualType: undefined, registers: { ...result.registers, '': deleted } }
            }
        ...
        if (raw === 'y') {
            ...
            return { ...state, mode: 'normal', visualStart: undefined, visualType: undefined, registers: { ...state.registers, '': yanked } }
        }
    ...
    if (raw === 'Ctrl+v') return { ...state, mode: 'visual', visualStart: { ...state.cursor }, visualType: 'block' }
 ...
 */
function getInsertedText(entryText: string, finalText: string, entryCursor: CursorPosition): string {
