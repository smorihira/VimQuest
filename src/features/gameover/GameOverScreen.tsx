...
export function GameOverScreen() {
  ...
  const collapsed = stage ? collapseSpells(spells) : []
  const overAmount = stage ? totalDamage - stage.life : 0
  const optimalDamage = stage?.stars[0] ?? 0
  ...
  }, [navigate, stage, actions, focusIdx])

  if (!stage) {
    return <div className="gameover-error">Stage not found: {stageId}</div>
  }
              ...
              {collapsed.map((entry, i) => {
                ...
                return (
                  <div key={i} className={`oplog-entry${isWaste ? ' waste' : ''}`}>
                    ...
                    {isWaste && <span className="oplog-waste-tag">{entry.damage}浪費</span>}
                  </div>
                )
              })}
            </div>
            <div className="oplog-summary">
              合計ダメージ: {totalDamage} / ライフ: {stage.life} →{' '}
              <span className="oplog-over">{overAmount}オーバー</span>
            </div>
          </div>
        )}
        ...
        {stage.hints.length > 0 && (
          <div className="suggestion">
            ...
            <div className="suggestion-text">
              最適解:{' '}
              <span className="suggestion-commands">{stage.hints[0].commands.join(' → ')}</span>
              <br />（{stage.hints[0].commands.length}コマンド = {stage.hints[0].commands.length}
              ダメージ）
            </div>
          </div>
        )}
