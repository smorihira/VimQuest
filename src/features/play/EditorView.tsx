...
export function EditorView({ state, goalText, goalCursor, showGoal, language }: EditorViewProps) {
    ...
    const visualRange = !showGoal && state.mode === 'visual' && state.visualStart
        ? getVisualRange(state)
        : null
...
function getVisualRange(state: EditorState): VisualRange | null {
    ...
    if (a.line < b.line || (a.line === b.line && a.col <= b.col)) {
        startLine = a.line; endLine = b.line; startCol = a.col; endCol = b.col
    } else {
        startLine = b.line; endLine = a.line; startCol = b.col; endCol = a.col
    }
...
function renderVisualLine(
    ...
    if (!line) return [<span key="empty" className="visual-selection">{'\u00A0'}</span>]
    ...
    result.push(<span key="s" className="visual-selection">{selected || '\u00A0'}</span>)
...
function renderVisualLineWithCursor(
    ...
    while (i < line.length) {
        ...
        const cls = [
            isSelected ? 'visual-selection' : '',
            isCursor && !isInsert ? cursorClass : '',
        ].filter(Boolean).join(' ')
        ...
        if (cls) {
            result.push(<span key={`c${i}`} className={cls}>{line[i]}</span>)
        } else {
    ...
    if (cursorCol >= line.length) {
        ...
        if (isInsert) {
            ...
            result.push(<span key="ec" className={cls}>{'\u00A0'}</span>)
        }
    }
